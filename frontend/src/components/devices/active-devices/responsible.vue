    ...
    <InfoFeed>
      <div class="inner-container">
        <select v-model="selected">
          <option value="" disabled hidden>Выберите отвественного за прибор</option>
          <option v-for="responsible in AVAILABLE_RESPONSIBLES" :value="responsible" :key="Math.random()">
      ...
      <Device
      v-if="DEVICES_BY_RESPONSIBLE.length"
      v-for="item in DEVICES_BY_RESPONSIBLE"
      :key="item.id"
      :device="item"
...
export default {
  ...
  methods: {
    ...mapActions([
      ...
      'GET_DEVICES_BY_RESPONSIBLE',
      'GET_AVAILABLE_RESPONSIBLES_ACTION'
    ]),
    openInfo(itemId) {
  ...
  mounted() {
    this.GET_AVAILABLE_RESPONSIBLES_ACTION()
  }
}
</script>
